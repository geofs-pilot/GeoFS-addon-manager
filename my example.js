      ...
      function createAddonManager() {
          ...
          function addAddon(name, runFunction) {
              ...
              const descriptions = {
                  ...
                  'HD and adblock': `Changes terrain imagery provider to Google Maps satellite data and removes ad banner`,
...
">
       ...
       window.hd = function() {
          (function() {
              //PASTE IN CODE BELOW
              window.geofsNewHDState=!0,window.executeOnEventDone("geofsStarted",function(){window.geofs.api.hdOn!==window.geofsNewHDState&&(window.jQuery("body").trigger("terrainProviderWillUpdate"),function e(){try{let r=document.querySelector("body > div.geofs-adbanner.geofs-adsense-container");r&&r.remove(),window.geofs.api.imageryProvider=new window.Cesium.UrlTemplateImageryProvider({maximumLevel:21,hasAlphaChannel:!1,subdomains:["mt0","mt1","mt2","mt3"],url:"https://{s}.google.com/vt/lyrs=s&x={x}&y={y}&z={z}"}),window.geofs.api.setImageryProvider(window.geofs.api.imageryProvider,!1),console.log("Imagery provider updated successfully.")}catch(t){console.error("Error updating imagery provider:",t)}}(),window.geofs.api.hdOn=window.geofsNewHDState,window.geofs.api.renderingQuality(),window.jQuery("body").trigger("terrainProviderUpdate"))}),window.executeOnEventDone("afterDeferredload",function(){!function e(r,t){fetch("https://cors-anywhere.herokuapp.com/"+r).then(e=>e.text()).then(t).catch(e=>console.error(`Error fetching ${r}:`,e))}("https://data.geo-fs.com/osm/{z}/{x}/{y}.png",function(e){window.geofs.mapXYZ=e,console.log("Map XYZ URL updated.")})});
          })();
      };
